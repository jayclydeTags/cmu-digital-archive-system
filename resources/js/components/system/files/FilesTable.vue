<template>
    <div>
        ...
        <v-card flat class="rounded-lg">
            ...
            <!-- FILES TABLE  -->
            <v-data-table :headers="headers" v-model="selected" :items="fetchFiles" :search="search" item-key="file_id"
                :single-select="singleSelect" :loading="isLoading" show-select class="elevation-1 table-striped">
                ...
                <template v-slot:item.file_status="{ item }">
                    ...
                    <div v-else>
                        <v-chip small :color="getColor(item.file_status)" dark>
                            {{ item.file_status }}
                        </v-chip>
                    </div>
                    ...
                    <v-toolbar flat>
                                ...
                                <div v-show="auth.user_type === 'Staff'">
                                    <v-btn-toggle v-model="icon" borderless>
                                        <v-btn value="left" color="info" v-bind="attrs" v-on="on">
                                            <v-icon class="text-white">mdi-plus-circle</v-icon>
                                            <span class="hidden-sm-and-down">New Record</span>
                                        </v-btn>
                            ...
                            <v-card>
                                <v-toolbar color="primary" dark>
                                    ...
                                    <v-container>
                                        <div v-show="auth.user_type === 'Chief'">
                                            <v-form ref="form" @submit.prevent="save">
                                                <v-alert text type="info">
                                                    Make sure to review the contents of the record throughly including its attached file before approving this record.
                                                </v-alert>
                                                <v-select v-model="form.file_status" prepend-inner-icon="mdi-archive"
                                                    :items="selectItem" item-value="value" item-text="text"
                                        ...
                                        <div v-show="auth.user_type === 'Staff'">
                                            <v-form ref="form" @submit.prevent="save">
                                                <div v-show=" formTitle === 'New Record' || formTitle === 'Update Record' ">

                                                    <v-text-field
                                                        v-show=" formTitle === 'New Record' || formTitle === 'Update Record'"
                                                    ...
                                                    <v-text-field v-show=" formTitle === 'New Record' || formTitle === 'Update Record' "
                                                        v-model="form.code"
                                                        :rules="rules.code"
                                                    ...
                                                    <v-alert text type="info" class="text-body-2">
                                                        Select Record Availability feature enable to classify the records accessibility. <br>
                                                        For records available for public access select PUBLIC, otherwise for confidential and permanent records select PRIVATE.
                                                    </v-alert>

                                                    <v-select v-show=" formTitle === 'New Record' || formTitle === 'Update Record' "
                                                    ...
                                                    <v-textarea v-show=" formTitle === 'New Record' || formTitle === 'Update Record' "
                                                    ...
                                                    <v-menu
                                                        ...
                                                        <v-date-picker v-model="date" :active-picker.sync="activePicker"
                                                            min="1900-01-01" @change="saveDate">
                                                        </v-date-picker>
                                                    </v-menu>
                                                </div>


                                                <!-- DISPLAY FOR FILE UPLOAD -->
                                                <div class="mb-3 " v-show="formTitle === 'File Location'">
                        ...
                        <v-dialog v-model="dialogDelete" max-width="450px">
                            <v-card>
                                ...
                                <v-container>
                                    <v-alert text type="info">
                                        Make sure you have deleted first the attached file for this record on the file location table else this action won't succeed.
                                    </v-alert>
                                    <h5>You are about to delete this Record. Do you to proceed?</h5>
                                </v-container>
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn-toggle v-model="icon">
                        <v-btn v-show="auth.user_type === 'Staff'" class="ma-1 rounded-sm" small color="info"
                            @click="editItem(item)">
                            <v-icon small class="text-white"> mdi-pencil-outline </v-icon>
                        </v-btn>
                        <v-btn v-show="auth.user_type === 'Staff'" class="ma-1 rounded-sm" small color="success"
                            @click="editFileLocation(item)">
                            <v-icon small class="text-white"> mdi-upload-outline </v-icon>
                        </v-btn>

                        <v-btn v-show="auth.user_type === 'Chief'" class="ma-1 rounded-sm" small color="info"
                            @click="editItem(item)">
                            <v-icon small class="text-white"> mdi-check-circle </v-icon>
                        </v-btn>
                    </v-btn-toggle>
                </template>
            </v-data-table>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                code: [(v) => !!v || "Reference Code is required"],
    ...
    computed: {
        ...
        formTitle() {
            ...
            //return this.editedIndex === -1 ? "New Record" : "Update Record";
        },
